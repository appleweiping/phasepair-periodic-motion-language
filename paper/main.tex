...
and 22 body joints, with actor/joint/frame masks. The group base uses a shared
512D actor encoder with no actor ID. In parallel, the periodic branch extracts
fixed, mask-aware body-motion activity from skeleton coordinates; it does not
apply Morlet kernels to learned base features. Motion is resampled to 20 Hz;
an independently hashed 20-Hz
...
\caption{Implementation architecture (no-result). Fixed motion activity and
the qualified learned group base are parallel paths. Directed relations
...
\label{eq:token}
\end{equation}
where $m_b$ is the band-valid mask and $C$ is the common postprocessor. The
capacity-matched pair-only control passes exact positive zero for $q_b$ through
the same $C$. Fixed-tree float64 microblock reduction, compensated
...
tower, or actor-0 reference. Every residual system reads the same immutable
winning-base checkpoint and cache for its seed.
