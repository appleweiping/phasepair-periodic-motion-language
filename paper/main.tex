...
% Engineering article template retained because no usable official conference
% paper kit is available in the repository. Re-template before submission.
\pdfminorversion=7
\documentclass[10pt,twocolumn]{article}
\usepackage[letterpaper,margin=0.58in,columnsep=0.21in]{geometry}
\usepackage{amsmath,amssymb,booktabs,array,balance,graphicx,microtype,xcolor}
...
\title{\vspace{-1.5em}PhaseSet: Permutation-Invariant Periodic Relation Tokens\\
for Multi-Person Motion--Language Retrieval}
\author{Anonymous Authors\\[-1pt]\small\DraftState}
\date{}
...
10 s auxiliary task may use machine-fused actor descriptions, visibly labeled
as such; the fusion model is barred from skeletons, spectra, phase, model
outputs, and split statistics.
...
checkpoints are frozen and shared across residual systems.
...
\textbf{Table \thetable:} Frozen full-gallery matrix; every result is
\ResultHold.\label{tab:hold}\\[2pt]
\scriptsize
\resizebox{\linewidth}{!}{%
...
\centering
\refstepcounter{table}
\textbf{Table \thetable:} Frozen slice/scaling report; dash is out of
scope.\label{tab:scaling}\\[2pt]
\scriptsize
\resizebox{\linewidth}{!}{%
\begin{tabular}{@{}clcccc@{}}
\toprule
...
are frozen, the protocol admits exactly one sealed-test execution. Inference
uses 100,000 shared paired capture-level bootstrap draws; captures, not
captions or windows, are units. The three test
...
For every H1--H8 comparison, the paired contribution is formed at capture
level after keeping all captions and windows of a capture together. Exactly
100,000 shared bootstrap index draws produce the point difference, percentile
...
One content-addressed aggregate renders JSON/CSV, LaTeX tables, figures, and
slides, preventing a manuscript number from diverging from the released
